...
namespace BytecodeApi.Extensions
{
	...
	public static class XmlExtensions
	{
		...
		{
			FormattedXmlWriterSettings = new XmlWriterSettings
			{
				Indent = true,
				IndentChars = "\t"
			};
		...
		{
			Check.ArgumentNull(xml, nameof(xml));
		...
		public static void SaveFormatted(this XDocument xml, string path)
		{
			Check.ArgumentNull(xml, nameof(xml));
			Check.ArgumentNull(path, nameof(path));
			Check.ArgumentNull(FormattedXmlWriterSettings, nameof(FormattedXmlWriterSettings));
		...
		{
			Check.ArgumentNull(element, nameof(element));
			Check.ArgumentNull(name, nameof(name));

			return element.Attribute(name) ?? new XAttribute(name, defaultValue);
		}
	}
}