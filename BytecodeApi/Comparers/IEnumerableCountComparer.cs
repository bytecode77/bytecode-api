...
{
	/// <summary>
	/// Represents a comparison operation that compares the result of the Count() method of <see cref="IEnumerable{T}" /> objects. Each call of <see cref="IEnumerableCountComparer{T}.Compare(object, object)" /> invokes the Count() method on the <see cref="IEnumerable{T}" />.
	/// </summary>
	/// <typeparam name="TSource">The type of the elements of the compared collections.</typeparam>
	public sealed class IEnumerableCountComparer<TSource> : IComparer, IComparer<IEnumerable<TSource>>
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="IEnumerableCountComparer{T}" /> class.
		/// </summary>
		public IEnumerableCountComparer()
		{
		}
