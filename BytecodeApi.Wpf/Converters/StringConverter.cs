...
public sealed class StringConverter : ConverterBase<string?, object?>
{
	...
	public override object? Convert(string? value, object? parameter)
	{
		return Method switch
		{
			...
			StringConverterMethod.Split => value?.Split(parameter?.ToString()),
			StringConverterMethod.SplitRemoveEmpty => value?.Split(parameter?.ToString(), StringSplitOptions.RemoveEmptyEntries),
			StringConverterMethod.SplitRemoveEmptyTrim => value?.Split(parameter?.ToString(), StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries),
